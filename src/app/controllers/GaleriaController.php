    ...
    public function galeria($request, $response, $args) {
        if (!isset($_SESSION['username'])) {
            return $response->withRedirect(
                $this->container->router->pathFor('login') . '?returnToUrl=' . $this->container->router->pathFor('galeria'), 303);
        }
        $title = "Galería";
        $info = $urlImagen = "";
        ...
        return $this->container->renderer->render($response, "galeria.view.php", compact('title', 'info', 'urlImagen', 'form', 'repositorio', 'imagenes'));
